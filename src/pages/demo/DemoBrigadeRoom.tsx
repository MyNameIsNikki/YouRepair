...
import { UploadModal } from '../../components/demo/UploadModal';
import { TicketsModal } from '../../components/demo/TicketsModal';
import { 
  ChevronRight, 
  ChevronLeft,
  Package,
...
export function DemoBrigadeRoom() {
  const { roomId } = useParams();
  const navigate = useNavigate();
  const [uploadModal, setUploadModal] = useState<{ isOpen: boolean; type: 'material' | 'media'; task: Task | null }>({
    isOpen: false,
    type: 'material',
    task: null
  });
  const [ticketsModal, setTicketsModal] = useState<{ isOpen: boolean; task: Task | null }>({
    isOpen: false,
    task: null
  });
  ...
  const handleUpload = (type: 'material' | 'media', task: Task) => {
    setUploadModal({ isOpen: true, type, task });
  };

  const handleViewTickets = (task: Task) => {
    setTicketsModal({ isOpen: true, task });
  };
    ...
    <div className="min-h-screen bg-[#F0EAD2]/20">
      ...
      <main className="pt-28 pb-16">
        <div className="container mx-auto px-4 lg:px-8 max-w-7xl">
          ...
          <div className="flex items-center gap-2 text-sm text-gray-600 mb-6">
            ...
            <Link to="/demo/brigade" className="hover:text-[#ADC178]">Демо для бригадира</Link>
            <ChevronRight className="w-4 h-4" />
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm mb-6">
            <div className="flex items-start justify-between">
              ...
              <div className="text-right">
                ...
                <p className="text-[#A98467]" style={{ fontSize: '24px', fontWeight: '700' }}>
              ...
              <Card key={task.id} className="p-6">
                <div className="flex items-start justify-between mb-4">
                    ...
                    <div className="flex flex-wrap gap-4 text-sm">
                      {task.assignedTo && (
                        <div className="flex items-center gap-2 text-gray-600">
                          <User className="w-4 h-4 text-[#A98467]" />
                          <span>{task.assignedTo}</span>
                        </div>
                      )}
                      {task.dueDate && (
                        <div className="flex items-center gap-2 text-gray-600">
                          <Calendar className="w-4 h-4 text-[#A98467]" />
                  ...
                  <div className="flex gap-2">
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={() => handleViewTickets(task)}
                      className="gap-2 relative"
                    >
                      <MessageSquare className="w-4 h-4" />
                      Тикеты
                      <div className="absolute -top-2 -right-2 w-5 h-5 bg-green-500 rounded-full flex items-center justify-center text-white text-xs" style={{ fontWeight: '700' }}>
                        0
                      </div>
                    </Button>
                  </div>
                ...
                <Tabs defaultValue="materials" className="w-full">
                  <TabsList className="w-full justify-start border-b border-[#DDE5B6] bg-transparent rounded-none h-auto p-0">
                    <TabsTrigger 
                      value="materials" 
                      className="rounded-none border-b-2 border-transparent data-[state=active]:border-[#A98467] data-[state=active]:bg-transparent"
                    >
                  ...
                  <TabsContent value="materials" className="mt-4">
                    <div className="mb-4">
                      <Button
                        onClick={() => handleUpload('material', task)}
                        className="bg-[#A98467] hover:bg-[#8B6D57] text-white gap-2"
                        size="sm"
                      >
                    ...
                    {task.materials.length > 0 ? (
                      <div className="space-y-3">
                        {task.materials.map((material) => (
                          <div key={material.id} className="bg-[#F0EAD2]/20 p-4 rounded-lg">
                            <div className="flex justify-between items-start">
                              ...
                              <div className="text-right">
                                <p className="text-[#A98467]" style={{ fontSize: '18px', fontWeight: '700' }}>
                        ...
                        <div className="bg-[#A98467]/10 p-3 rounded-lg flex justify-between items-center border border-[#A98467]">
                          <span className="text-[#6C584C]" style={{ fontWeight: '600' }}>
                            Итого:
                          </span>
                          <span className="text-[#A98467]" style={{ fontSize: '18px', fontWeight: '700' }}>
                  ...
                  <TabsContent value="media" className="mt-4">
                    <div className="mb-4">
                      <Button
                        onClick={() => handleUpload('media', task)}
                        className="bg-[#A98467] hover:bg-[#8B6D57] text-white gap-2"
                        size="sm"
                      >
      ...
      {uploadModal.isOpen && uploadModal.task && (
        <UploadModal
          ...
          onClose={() => setUploadModal({ isOpen: false, type: 'material', task: null })}
          type={uploadModal.type}
          taskName={uploadModal.task.name}
        />
      )}

      {ticketsModal.isOpen && ticketsModal.task && (
        <TicketsModal
          isOpen={ticketsModal.isOpen}
          onClose={() => setTicketsModal({ isOpen: false, task: null })}
          taskName={ticketsModal.task.name}
        />
      )}
